<?php 
	...
	$p = isset($_REQUEST['point']) ? urldecode($_POST['point']) : ".";
	
	$d = isset($_REQUEST['date']) ? $_REQUEST['date'] : "8";
	...
	{
		if($d == '8'){
			$res = mysql_query("SELECT b.num_iid,a.points,a.sn FROM ".TABLE_D." a LEFT JOIN ".TABLE_L." b on a.sn=b.sn WHERE a.status != 1 ");
		}else
		{
			$res = mysql_query("SELECT b.num_iid,a.points,a.sn FROM ".TABLE_D." a LEFT JOIN ".TABLE_L." b on a.sn=b.sn WHERE a.status = 1 ");
		}
		while($v = mysql_fetch_row($res))
		{
			...
			}else
			{
				if($d == '8')
				{
					mysql_query("UPDATE ".TABLE_D." SET `status`='1' WHERE `sn`='".$v[2]."' LIMIT 1");
				}else
				{
					mysql_query("UPDATE ".TABLE_D." SET `status`='2' WHERE `sn`='".$v[2]."' LIMIT 1");
				}
			}
		}
		theEnd($stamp);
	...
	{
		//我叫杰拉德  我03ZHU/HABZ11feKs+8vZNA15zvl/T3NRed4K8gNQjGPHMIHExKqzxd1gHVAWPTmYqO
		//雅01fDTSJV4jHmQc22joNxN72M2DEMds1FAQI7fgfrP3PMg=
		$res = getTradesSold("我03ZHU/HABZ11feKs+8vZNA15zvl/T3NRed4K8gNQjGPHMIHExKqzxd1gHVAWPTmYqO");
		$nums = $res['total_results'];
		if($nums == '0')
		{
			echo '无消费记录';
		}else
		{
			print_r($res['trades']['trade']);
		}
		die;
	}
	elseif($f == "app")
	...
	elseif($f == "send")
	{
		...
		//$r6 = sendPromotionCouponSns("99256155","q01GLZPQn84fGe4HLWpsfRsQ2MAESw3U35t+ASd0SOXQCA=");
	...
	elseif($f == "point")
	{
		if($b == 'kids')
		{
			$thePoint = "gxgkids祝大家2014  马上缘梦";
		}elseif($b == 'gxg')
		{
			$thePoint = ".";
		}elseif($b == '1978')
		{
			$thePoint = "春装新品热卖中，全场5-7折。今日活动: 1.满399元减30元并包邮，满798元减60元加送时尚T恤。满减上不封顶。2.买标题带[赠衬衫]西装加送衬衫一件，随机赠送，数量有限。这个春天与你一起分享。";
		}else
		{
			$thePoint = ".";
		}
	...
	elseif($f == "offline")
	{
		...
		while($v = mysql_fetch_row($res))
		{
			...
			{
				mysql_query("UPDATE ".TABLE." SET `status`=2 WHERE `sn`='".$v[1]."' LIMIT 1");
			}
		}
		theEnd($stamp);
		exit;
	...
	function getUmp($iid)
	{
		...
		if($ump_price == '')
		{
			$ump_price = @$ump['promotions']['promotion_in_item']['promotion_in_item'][0]['item_promo_price'];
		}
		if($ump_price == '')
		{
			$ump_price = @$ump['promotions']['promotion_in_item']['promotion_in_item']['item_promo_price']['sku_price_list']['price'][0];
		}
	...
	function echoPop($pops)
	{
		foreach($pops as $k=>$v)
		{
			if(isset($v['stockList']))
			{
				...
				{
					echo $v['itemCode'].",".$v['itemPrice'].",".$vv['stockLocalCode'].",".$vv['stockCount']."<br>";
				}
			}
		}
	}
